...
const updateProfile = async (req, res) => {
  try {
    ...
    sendResponse(res, 200, true, 'Cập nhật profile thành công', { user: updatedUser });
...
const updateUser = async (req, res) => {
  try {
    ...
    sendResponse(res, 200, true, 'Cập nhật người dùng thành công', { user: updatedUser });
...
const deleteUser = async (req, res) => {
  try {
    ...
    if (user._id.toString() === req.user.id) {
      return sendResponse(res, 400, false, 'Không thể xóa tài khoản của chính mình');
    }
