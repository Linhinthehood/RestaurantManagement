...
import { FaUserTie, FaUser, FaUtensils, FaUserClock, FaEye, FaEyeSlash } from 'react-icons/fa';


const roles = [
  { value: 'Manager', label: 'Quản lý', icon: <FaUserTie /> },
  { value: 'Waiter', label: 'Phục vụ', icon: <FaUser /> },
  { value: 'Chef', label: 'Đầu bếp', icon: <FaUtensils /> },  // <-- Thay đổi đây
...
export default function RegisterUserPage() {
  ...
  const validate = () => {
    ...
    if (!form.phoneNumber.match(/^[0-9]{10,11}$/)) errs.phoneNumber = 'Số điện thoại không hợp lệ';

    if (form.dob && new Date(form.dob) > new Date()) errs.dob = 'Ngày sinh không thể là tương lai';
  ...
  return (
    <div className="max-w-lg mx-auto mt-10 p-6 bg-white rounded shadow-md">
      <h2 className="text-3xl mb-6 font-semibold text-center">Đăng ký tài khoản</h2>

      {message && (
        <div className={`mb-4 p-3 rounded text-white text-center ${
          message.toLowerCase().includes('thành công') ? 'bg-green-500' : 'bg-red-500'
        }`}>
          {message}
        </div>
      )}
      ...
      <form onSubmit={onSubmit} noValidate>
        ...
        <div className="mb-4">
          ...
          />
          {errors.dob && <p className="text-red-600 mt-1 text-sm">{errors.dob}</p>}
        </div>
        ...
        <div className="mb-4">
          ...
          <Select
            ...
            classNamePrefix={errors.gender ? 'react-select-error' : 'react-select'}
          />
          {errors.gender && <p className="text-red-600 mt-1 text-sm">{errors.gender}</p>}
        </div>
        ...
        <div className="mb-6">
          ...
          <Select
            ...
            classNamePrefix={errors.role ? 'react-select-error' : 'react-select'}
          />
          {errors.role && <p className="text-red-600 mt-1 text-sm">{errors.role}</p>}
        </div>

        {/* Avatar */}
        <div className="mb-6">
          <label className="block mb-1 font-medium">Link avatar (tùy chọn)</label>
          <input
            name="avatar"
            type="url"
            value={form.avatar}
